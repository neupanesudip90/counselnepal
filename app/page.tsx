    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <h1 className="font-display text-5xl lg:text-6xl font-extrabold text-dark leading-tight mb-4">
                Your Future
                <br />
                <span className="text-primary">Starts With</span>
                <br />
                The Right Guide
              </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-8">
                  ...
                  <select className="w-full pl-9 pr-4 py-3 border border-border rounded-xl text-sm text-dark bg-white appearance-none focus:outline-none focus:border-primary transition-colors">
                    ...
                    {countries.map((c) => (
                      <option key={c.code} value={c.name}>
                        {c.flag} {c.name} — {c.counsellors} counsellors
                      </option>
                    ))}
                  </select>
                </div>
                <Link
                  href="/counsellors"
                  className="bg-primary text-white px-6 py-3 rounded-xl font-semibold text-sm hover:bg-blue-700 transition-colors flex items-center gap-2 justify-center"
                >
            ...
            <div className="hidden lg:block relative">
              {/* Main consultancy card */}
              <div
                className="glass rounded-3xl p-8 shadow-2xl max-w-sm mx-auto transition-all duration-700"
                style={{ borderTop: `4px solid ${hero?.color}` }}
              >
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="font-display font-bold text-dark">{hero?.name}</h3>
                    <p className="text-slate text-sm">{hero?.location}</p>
                    <div className="flex items-center gap-1 mt-1">
                      <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                      <span className="text-xs font-semibold">{hero?.rating}</span>
                      <span className="text-xs text-slate">({hero?.reviews} reviews)</span>
                    </div>
                  </div>
                </div>
                <p className="text-slate text-sm italic mb-5">"{hero?.tagline}"</p>
                <div className="flex flex-wrap gap-2 mb-5">
                  {hero?.countries.map((c) => (
                    <span key={c} className="px-2.5 py-1 bg-surface text-slate text-xs rounded-full border border-border">
                      {c}
                    </span>
                  ))}
                </div>
                <Link
                  href="/counsellors"
                  className="block text-center bg-primary text-white font-semibold text-sm py-3 rounded-xl hover:bg-blue-700 transition-colors"
                >
                  View Counsellors
                </Link>
              </div>
                ...
                {heroConsultancies.map((_, i) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all ${
                      i === heroIndex ? 'w-6 bg-primary' : 'w-1.5 bg-border'
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setCardIndex((i) => (i - 1 + featuredConsultancies.length) % featuredConsultancies.length)}
              ...
              <button
                onClick={() => setCardIndex((i) => (i + 1) % featuredConsultancies.length)}
                className="w-9 h-9 rounded-full bg-primary text-white flex items-center justify-center hover:bg-blue-700 transition-colors"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {featuredConsultancies.map((c, i) => {
              ...
              return (
                <div
                  key={c.id}
                  className={`bg-white rounded-2xl p-6 border-2 transition-all duration-500 card-hover ${
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} className={`w-3 h-3 ${s <= Math.floor(c.rating) ? 'text-amber-400 fill-amber-400' : 'text-border'}`} />
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {c.countries.map((cn) => (
                      <span key={cn} className="px-2 py-0.5 bg-surface text-slate text-xs rounded-full">{cn}</span>
                    ))}
                  </div>
                  <Link
                    href="/counsellors"
                    className="block text-center text-sm font-semibold text-primary border border-primary/20 py-2 rounded-xl hover:bg-primary hover:text-white transition-all"
                  >
                    View Counsellors
                  </Link>
                </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {countries.map((c) => (
              <Link
                ...
                className="group bg-surface border border-border rounded-2xl p-5 text-center hover:border-primary hover:shadow-lg hover:shadow-primary/10 transition-all card-hover"
              >
      ...
      <section className="py-16 bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/counsellors" className="text-sm text-primary font-semibold flex items-center gap-1 hover:gap-2 transition-all">
      ...
      <section className="py-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <h2 className="font-display font-extrabold text-dark text-4xl mb-4">
            Ready to Start Your
            <br />
            <span className="text-primary">International Journey?</span>
          </h2>
          <p className="text-slate mb-8">Join thousands of Nepali students who found their path through CounselNepal.</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/login"
              className="bg-primary text-white px-8 py-3.5 rounded-xl font-semibold hover:bg-blue-700 transition-colors flex items-center gap-2 justify-center"
            >
              Get Started Free <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              href="/counsellors"
              className="border border-border text-dark px-8 py-3.5 rounded-xl font-semibold hover:border-primary hover:text-primary transition-colors"
            >
              Browse Counsellors
            </Link>
          </div>
